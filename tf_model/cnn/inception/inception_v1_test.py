...
class InceptionV1Test(tf.test.TestCase):
    ...
    def testBuildOnlyUptoFinalEndpoint(self):
        ...
        for index, endpoint in enumerate(endpoints):
            with tf.Graph().as_default():
                inputs = tf.random_uniform((batch_size, height, width, 3))
                out_tensor, end_points = inception_v1.inception_v1_base(inputs, final_endpoint=endpoint)
    ...
    def testBuildAndCheckAllEndPointsUptoMixed5c(self):
        ...
        inputs = tf.random_uniform((batch_size, height, width, 3))
        _, end_points = inception_v1.inception_v1_base(inputs, final_endpoint='Mixed_5c')
    ...
    def testGlobalPoolUnknownImageShape(self):
        ...
        with self.test_session() as sess:
            inputs = tf.placeholder(tf.float32, shape=(batch_size, None, None, 3))
            logits, end_points = inception_v1.inception_v1(inputs, num_classes, global_pool=True)
            self.assertTrue(logits.op.name.startswith('InceptionV1/Logits'))
            self.assertListEqual(logits.get_shape().as_list(), [batch_size, num_classes])
            pre_pool = end_points['Mixed_5c']
            feed_dict = {inputs: input_np}
    ...
    def testEvaluation(self):
        ...
        logits, _ = inception_v1.inception_v1(eval_inputs, num_classes, is_training=False)
