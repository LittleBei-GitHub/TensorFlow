...
def xception(inputs, num_classes=1001, is_training=True, scope='xception'):
    ...
    with tf.variable_scope('Xception') as sc:
        ...
        with slim.arg_scope([slim.separable_conv2d], depth_multiplier=1), \
            ...
            net = slim.separable_conv2d(net, 128, [3, 3], scope='block2_dws_conv2') # depthwise separable convolution
            ...
            net = slim.separable_conv2d(net, 256, [3, 3], scope='block3_dws_conv2') # depthwise separable convolution
            ...
            net = slim.separable_conv2d(net, 728, [3, 3], scope='block4_dws_conv2') # depthwise separable convolution
            ...
            for i in range(8):
                ...
                net = slim.separable_conv2d(net, 728, [3, 3], scope=block_prefix + 'dws_conv1') # depthwise separable convolution
                net = slim.batch_norm(net, scope=block_prefix + 'bn1')
                net = tf.nn.relu(net, name=block_prefix + 'relu2')
                net = slim.separable_conv2d(net, 728, [3, 3], scope=block_prefix + 'dws_conv2') # depthwise separable convolution
                net = slim.batch_norm(net, scope=block_prefix + 'bn2')
                net = tf.nn.relu(net, name=block_prefix + 'relu3')
                net = slim.separable_conv2d(net, 728, [3, 3], scope=block_prefix + 'dws_conv3') # depthwise separable convolution
...
def xception_arg_scope(weight_decay=0.00001, batch_norm_decay=0.9997, batch_norm_epsilon=0.001):
    '''
    ...
    with slim.arg_scope([slim.conv2d, slim.separable_conv2d], weights_regularizer=slim.l2_regularizer(weight_decay),
                        biases_initializer=None, activation_fn=None):
        # Set parameters for batch_norm. Note: Do not set activation function as it's preset to None already.
        with slim.arg_scope([slim.batch_norm], decay=batch_norm_decay, epsilon=batch_norm_epsilon) as scope:
            return scope