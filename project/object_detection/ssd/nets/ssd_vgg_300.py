...
def ssd_anchor_one_layer(img_shape,
    ...
    num_anchors = len(sizes) + len(ratios)  # 每个位置上的锚框数量
    h = np.zeros((num_anchors, ), dtype=dtype)
    w = np.zeros((num_anchors, ), dtype=dtype)
    # Add first anchor boxes with ratio=1.
    h[0] = sizes[0] / img_shape[0]  # size相当于计算好的sk
...
def ssd_multibox_layer(inputs,
    ...
    # Location.
    num_loc_pred = num_anchors * 4  # 定位通道数
    ...
    # Class prediction.
    num_cls_pred = num_anchors * num_classes  # 预测通道数
