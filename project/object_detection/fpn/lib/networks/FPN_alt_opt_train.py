...
class FPN_alt_opt_train(Network):
    def __init__(self, trainable=True):
        ...
        self.bbox_inside_weights = tf.placeholder(tf.float32, shape=[None, self.n_classes * 4],
                                                  name='bbox_inside_weights')
        self.bbox_outside_weights = tf.placeholder(tf.float32, shape=[None, self.n_classes * 4],
    ...
    def setup(self):
        ...
        anchor_size = [None, None, 32, 64, 128, 256,
                       512]  # P6 should be in RPN, but not Fast-RCNN, according to the paper
    ...
    def build_RPN_loss(self, vs_name):
        ...
        if cfg.TRAIN.WEIGHT_DECAY > 0:
            ...
            print regularization_losses
            regularization_losses = [v for v in regularization_losses for vs in vs_name if
                                     v.op.name.startswith(vs + '/')]
