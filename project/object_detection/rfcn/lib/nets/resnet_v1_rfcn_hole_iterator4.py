...
def bottleneck_hole(inputs,
    ...
    with variable_scope.variable_scope(scope, 'bottleneck_v1', [inputs]) as sc:
        ...
        residual = layers_lib.conv2d(residual, depth_bottleneck, [3, 3], stride=1, rate=rate, padding='SAME',
                                     scope='conv2')
        residual = layers.conv2d(
...
class resnetv1(Network):
    ...
    def build_network(self, sess, input_rois=None, roi_scores=None, is_training=True):
        ...
        assert (0 <= cfg.RESNET.FIXED_BLOCKS < 4)  # 被固定的块的数量
        ...
        with tf.variable_scope(self._resnet_scope, self._resnet_scope):
            ...
            rfcn_net_classes = slim.conv2d(rfcn_net, cfg.K * cfg.K * (20 + 1), [1, 1],
                                           weights_initializer=tf.random_normal_initializer(stddev=0.01),
                                           weights_regularizer=slim.l2_regularizer(scale=0.0005),
                                           scope='refined_classes',
                                           activation_fn=None)
            rfcn_net_bbox = slim.conv2d(rfcn_net, cfg.K * cfg.K * 4 * 21, [1, 1],
                                        weights_regularizer=slim.l2_regularizer(scale=0.0005),
                                        weights_initializer=tf.random_normal_initializer(stddev=0.01),
                                        scope='refined_bbox',
                                        activation_fn=None)
    ...
    def get_variables_to_restore(self, variables, var_keep_dic):
        ...
        for v in variables:
            ...
            if (v.name == ('rpn_network/' + self._resnet_scope + '/conv1/weights:0')) or (
                    v.name == ('rfcn_network/' + self._resnet_scope + '/conv1/weights:0')):
