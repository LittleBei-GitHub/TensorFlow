...
class Network(object):
    ...
    def _smooth_l1_loss_vector(self, bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0,
        ...
        loss_box = tf.Print(loss_box, [tf.shape(bbox_pred), tf.shape(bbox_targets), tf.shape(bbox_inside_weights),
                                       tf.shape(box_diff), tf.shape(in_box_diff), tf.shape(smoothL1_sign)],
                            'smooth l1 loss outputs')
        return loss_box
    ...
    def _add_focal_losses(self, sigma_rpn=3.0):
        with tf.variable_scope('loss_' + self._tag) as scope:
            ...
            cross_entropy = tf.reduce_mean(
                -alpha_t * tf.pow(1 - predictions_pt, gamma) * tf.log(predictions_pt + epsilon))
