...
class SolverWrapper(object):
    ...
    def train_model(self, sess, max_iters):
        ...
        with sess.graph.as_default():
            ...
            rpn_layers, proposal_targets = self.net.create_architecture(sess, 'TRAIN', self.imdb.num_classes,
                                                                        scope='rpn_network', tag='default',
                                                                        anchor_scales=cfg.ANCHOR_SCALES,
                                                                        anchor_ratios=cfg.ANCHOR_RATIOS)
            rfcn_layers, _ = self.rfcn_network.create_architecture(sess, 'TRAIN', self.imdb.num_classes,
                                                                   scope='rfcn_network', tag='default',
                                                                   anchor_scales=cfg.ANCHOR_SCALES,
                                                                   anchor_ratios=cfg.ANCHOR_RATIOS,
        ...
        if lsf == 0:
            ...
            variables_to_restore_rpn, variables_to_restore_rfcn = self.net.get_variables_to_restore(variables,
                                                                                                    var_keep_dic)
        ...
        while iter < 200001:
            ...
            if iter % 20 == 0:
                print('iter: %d / %d, stage: %s, total loss: %.6f\n >>> rpn_loss_cls: %.6f\n '
                      '>>> rpn_loss_box: %.6f\n >>> loss_cls: %.6f\n >>> loss_box: %.6f\n >>> lr: %f' % \
                      (iter, max_iters, stage_infor, total_loss, rpn_loss_cls, rpn_loss_box, loss_cls, loss_box,
                       lr.eval()))
