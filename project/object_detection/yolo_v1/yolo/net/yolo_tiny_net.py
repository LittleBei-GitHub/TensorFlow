...
class YoloTinyNet(Net):
    ...
    def body1(self, num, object_num, loss, predict, labels, nilboy):
        ...
        objects = tf.pad(objects, temp, "CONSTANT")  # pad()填充，temp记录了填充范围
        ...
        base_boxes = np.tile(np.resize(base_boxes, [self.cell_size, self.cell_size, 1, 4]),
                             [1, 1, self.boxes_per_cell, 1])  # tile()复制
        # 计算预测中心点的坐标以及宽和高
    ...
    def loss(self, predicts, labels, objects_num):
        ...
        for i in range(self.batch_size):
            predict = predicts[i, :, :, :]
            label = labels[i, :, :]  # 一张图片的label
            object_num = objects_num[i]
            nilboy = tf.ones([7, 7, 2])
            tuple_results = tf.while_loop(self.cond1, self.body1, [tf.constant(0),
                                                                   object_num,
                                                                   [class_loss, object_loss, noobject_loss, coord_loss],
                                                                   predict,
                                                                   label,
                                                                   nilboy])  # while_loop()相当于while循环
            for j in range(4):
