...
def main(FLAGS):
    ...
    '''--------Creat palceholder--------'''
    datas, labels, train = net.create_placeholder(FLAGS.batch_size, FLAGS.width, FLAGS.height, scale_width,
                                                  scale_height)
        ...
        for epoch in range(FLAGS.epoch):
            ...
            for i in range(len(train_filenames)):
                ...
                _, batch_loss, rs = sess.run([optimizer, loss, merged],
                                             feed_dict={datas: normalize_datas, labels: train_labels[i], train: True})
