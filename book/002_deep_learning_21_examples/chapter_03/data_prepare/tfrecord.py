...
def _process_image_files_batch(coder, thread_index, ranges, name, filenames,
    ...
    for s in range(num_shards_per_batch):
        # 生成文件名分片版本号, e.g. 'train-00002-of-00010'
...
def _process_image_files(name, filenames, texts, labels, num_shards, command_args):
    ...
    使用的是多线程多分片的方式
    Args:
      name: string, 数据集类型的唯一标识
      filenames: list of strings; 每个字符串是一个图像文件的路径
      texts: list of strings; 每个字符串是一个人可识别的类别名称
      labels: list of integer; 每个整数是一个图像的真实标签
      num_shards: 数据集被分割的块数
    """
    assert len(filenames) == len(texts)
    assert len(filenames) == len(labels)

    # 将所有的图像分成具有[ranges[i][0], ranges[i][1]]格式的批次
    spacing = np.linspace(0, len(filenames), command_args.num_threads + 1).astype(
        np.int)  # numpy.linspace()将指定的间隔平均分成n-1份
    ranges = []
    ...
    sys.stdout.flush()  # 刷新，实在控制台可以显示上面的信息
    ...
    for thread_index in range(len(ranges)):
        args = (coder, thread_index, ranges, name, filenames, texts, labels, num_shards, command_args)
...
def _process_dataset(name, directory, num_shards, labels_file, command_args):
    """
    处理一个完整的数据集并将其保存为TFRecord格式
    Args:
      name: string, 识别数据集类型的唯一标识
      directory: string, 数据集所在的根目录
      num_shards: 数据集的块数
      labels_file: string, 标签文件路径
    """
