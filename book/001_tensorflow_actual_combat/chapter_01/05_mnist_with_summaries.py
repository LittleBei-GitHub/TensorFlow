...
def train():
    ...
    def nn_layer(input_tensor, input_dim, output_dim, layer_name, act=tf.nn.relu):
        with tf.name_scope(layer_name):
            with tf.name_scope('weights'):
                ...
                variable_summaries(weights)  # 权重的summary
            with tf.name_scope('biases'):
                biases = bias_variable([output_dim])
                variable_summaries(biases)   # 偏置的summary
    ...
    with tf.name_scope('dropout'):
        ...
        tf.summary.scalar('dropout_keep_probability', keep_prob)  # 将dropout的keep_prob放入summary
        dropped = tf.nn.dropout(hidden1, keep_prob)               # 执行dropout
    ...
    ## 定义train相关的summary
    train_writer = tf.summary.FileWriter(FLAGS.log_dir + '/train', sess.graph)  # 使用sess.graph可以删除之前的结果
    ## 定义test相关的summary
    ...
    for i in range(FLAGS.max_steps):
        if i % 10 == 0:
            ...
            test_writer.add_summary(summary, i)   # 往test_writer中添加summary
