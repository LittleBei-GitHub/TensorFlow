    ...
    with tf.Session() as sess:
        ...
        for epoch in range(training_epochs):
            ...
            for batch_idx in range(total_batches):
                ...
                # 运行优化器Train节点（backprop)
                sess.run(trainer, feed_dict={X_origin: batch_x,
                                             Y_true:batch_y,
                                             learning_rate:learning_rate_init})
                #每调用一次训练节点，training_step就加1，最终==training_epochs*total_batches
                training_step += 1

                # 每训练display_step次，计算当前模型的损失和分类准确率
                if training_step % display_step == 0:
                    #计算当前模型在目前（最近）见过的display_step个batch_size的训练集上的损失和分类准确率
